...
z_low_sharpening = ones(1, length(x_sharpen)) * -0.05;
z_low_sharpening(mid) = 2;

z_high_sharpening = ones(1, length(x_sharpen)) * -0.15;
z_high_sharpening(mid) = 2;
...
y_low_sharpening = y_low_sharpening / max(abs(y_low_sharpening), [], 'all');
y_high_sharpening = y_high_sharpening / max(abs(y_high_sharpening), [], 'all');
y_low_smoothing  = y_low_smoothing  / max(abs(y_low_smoothing), [], 'all');
y_high_smoothing = y_high_smoothing / max(abs(y_high_smoothing), [], 'all');
